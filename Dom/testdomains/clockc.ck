...
factory(oc,ord)
   ...
   uint32 ord;
{
   KALL(COMPONENTS,COMPCONSOLE) KEYSTO(CONSOLE);
   KALL(CONSOLE,0) KEYSTO(,CONSOLE) RCTO(returncode);
...
//       oc=dofreeze();  
   ...
   KALL(SB,SB_CreateNode) KEYSTO(MEMNODE);
   KALL(MEMNODE,Node_MakeNodeKey) STRUCTFROM(ndb7) KEYSTO(MEMNODE);
   KALL(DOMKEY, Domain_GetMemory) KEYSTO(K0);
   KALL(MEMNODE,Node_Swap+0) KEYSFROM(K0);
   KALL(DOMKEY, Domain_SwapMemory) KEYSFROM(MEMNODE);
 
   KALL(COMPONENTS,COMPKERNELP) KEYSTO(K0);
   KALL(MEMNODE,Node_Swap+8) KEYSFROM(K0);
   KALL(COMPONENTS,COMPSYSTIME) KEYSTO(SYSTIME);
   KALL(DOMKEY, Domain_MakeStart) KEYSTO (SK);
   timezone_offset.hi  = 0;
   timezone_offset.low = 0;
   memcpy (timezone_code, "UT ", 3 );
   returncode = 0;
   LDEXBL (CALLER, returncode) KEYSFROM (SK);
 
   for (;;)
   { LDENBL  OCTO (ordercode)
             CHARTO (param.String,8,paramlen)
             KEYSTO (,,,CALLER)
             DBTO (databyte);
     RETJUMP ();
     ...
     LDEXBL  (CALLER,returncode) CHARFROM (param.String, paramlen);
    }     /* end for (;;) */

   KC (MEMNODE, Node_Fetch+0) KEYSTO(K0);       // put memory back
   KC (DOMKEY, Domain_SwapMemory) KEYSFROM(K0);
   KC (SB, SB_DestroyNode) KEYSFROM(MEMNODE);

   return 0;
  }       /* end of main  */

dofreeze() {
   KC (CONSOLE,0) CHARFROM("Freezing\n",9) RCTO(returncode);
//   freezedry();

   KALL(COMPONENTS,COMPCONSOLE) KEYSTO(CONSOLE);
   KALL(CONSOLE,0) KEYSTO(,CONSOLE) RCTO(returncode);

   KC (CONSOLE,0) CHARFROM("Thawing\n",8) RCTO(returncode);
}
...
{  
   ...
   KALL (CONSOLE,0) CHARFROM(buf,strlen(buf)) RCTO(returncode);
...
{  new_databyte = databyte;
   ...
   KALL (DOMKEY, Domain_MakeStart) CHARFROM (&new_databyte, 1)
                                   RCTO (returncode)
                                   KEYSTO (SK);
   LDEXBL (CALLER, returncode) KEYSFROM (SK);
 }             /* end of get_new_key   */
...
LLI *ctod1()  /* returns offset TOD (S370 time) */
{
   ...
   if(llicmp(&restarttod,&(KP->KP_RestartTOD))) {
      KC (COMPONENTS,COMPCALCLOCK) KEYSTO(K0);
      KC (K0,8) CHARTO(caltime,8);
      rc=cal2tod(caltime,&calsysoffset);
//      if(rc) KC(COMPONENTS,1024+rc) CHARFROM(caltime,8);
      KC (SYSTIME,7) STRUCTTO(tod);
