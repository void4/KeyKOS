...
#define IOCANCELLED   0x00200000
   ...
   KC (caller,EXTEND_OC) KEYSTO(k0,k1,k2,caller) RCTO(factoc);

   if(factoc) exit(INVALIDOC_RC);   /* must be zero */

   flags = 0;

   KC (domkey,Domain_GetMemory) KEYSTO(siksok);
   KC (siksok,Node_Swap+NODESIK) KEYSFROM(k0) KEYSTO(,,k0);
   KC (siksok,Node_Swap+NODESOK) KEYSFROM(k1) KEYSTO(,,k1);
   KC (siksok,Node_Swap+NODECCK) KEYSFROM(k2) KEYSTO(,,k2);
...
**********************************************************************************************/
   ...
   KC (domkey,Domain_GetMemory) KEYSTO(siksok);
   KC (siksok,Node_Swap+NODESIK) KEYSTO(siksok);
   ...
   if(!fork()) {   /* reader domain */
       ...
       while(1) {
          ...
          KC (siksok,8192+255) CHARTO(buf,255,len) KEYSTO(,,,siksok) RCTO(rc);
//sprintf(pbuf,"SIK rc = %X len=\n",rc,len);
//outsok(pbuf);
...
**********************************************************************************************/
   ...
   KC (domkey,Domain_MakeStart) STRUCTFROM(ddb) KEYSTO(central);
   KC (domkey,Domain_GetMemory) KEYSTO(siksok);
   KC (siksok,Node_Fetch+NODESOK) KEYSTO(siksok);
   if(!fork()) {
      KC (domkey,Domain_GetKey+comp) KEYSTO(,,caller);       /* zap caller key */
//      KC (siksok,0) KEYSTO(,,,siksok) RCTO(rc);
...
//outsok(pbuf);
         KC (siksok,0) CHARFROM(buf,len) KEYSTO(,,,siksok) RCTO(rc);
//sprintf(pbuf,"SIKSOK return code %X\n",rc);
//outsok(pbuf);
         if(rc == KT+1) break;
      }
      exit(0);
   ...
   KC (comp,0) KEYSTO(,,siksok);  /* HACK, why is this necessary... what is going on */
                                  /* without this the SOK key (or SIK) is used by */
                                  /* someone and burned */
...
//outsok(pbuf);
            ...
            if(flags & HAVEREADER) {  /* send if off */
               /* send the reader off to do the work */
//outsok("Sending reader off\n");
               readdior=parm.dior;
